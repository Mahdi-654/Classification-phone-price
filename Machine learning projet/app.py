...
# Charger les données avec le nouveau système de mise en cache
@st.cache_data
def load_data():
...
def main():
    ...
    if app_mode == "Prédiction":
        ...
        if st.button('Prédire la plage de prix'):
            if st.session_state.model is None:
                st.warning("Les métriques ne sont pas encore disponibles. Veuillez entraîner le modèle d'abord.")
            else:
                prediction = predict_price(st.session_state.model, user_input_df)
                st.write(f"La plage de prix prédite est : {prediction}")
